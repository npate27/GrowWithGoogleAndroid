            ...
            case CODE_WEATHER:
                ...
                try {
                    for (ContentValues cv : values) {
                        long currentWeatherDate = cv.getAsLong(WeatherContract.WeatherEntry.COLUMN_DATE);
                        if (!SunshineDateUtils.isDateNormalized(currentWeatherDate)) {
                            throw new IllegalArgumentException("Date is not normalized and can't be inserted");
                        }
