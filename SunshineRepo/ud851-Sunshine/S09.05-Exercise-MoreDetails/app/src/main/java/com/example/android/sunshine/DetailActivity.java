...
//  COMPLETED (20) Create a constant int to identify our loader used in DetailActivity
    public static final String[] WEATHER_DETAILS_PROEJECTION = {
...
//  COMPLETED (11) Declare TextViews for the date, description, high, low, humidity, wind, and pressure
    private TextView mWeatherDate, mDescription, mHigh, mLow, mHumidity, mWind, mPressure;
    @Override
...
//      COMPLETED (13) Find each of the TextViews by ID
        mWeatherDate = (TextView) findViewById(R.id.tv_selected_date);
...
//      COMPLETED (35) Initialize the loader for DetailActivity
        ...
        mUri = intent.getData();
        if(mUri == null){
            throw new NullPointerException("Uri passed via intent is null");
        }
    ...
    public Loader<Cursor> onCreateLoader(int id, Bundle args) {
        switch (id){
            case WEATHER_LOADER_ID:
                return new CursorLoader(this, mUri, WEATHER_DETAILS_PROEJECTION,
                        null,null, null);
            default:
                throw new RuntimeException("Loader not initialized with id: " + id);
        }
    }
...
//      COMPLETED (33) Store a forecast summary in mForecastSummary
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        if(data != null && data.moveToFirst()){
            ...
            mHigh.setText(SunshineWeatherUtils.formatTemperature(this, highInCelsius));

            double lowInCelsius = data.getDouble(INDEX_WEATHER_MIN_TEMP);
            mLow.setText(SunshineWeatherUtils.formatTemperature(this, lowInCelsius));

            double humidity = data.getDouble(INDEX_WEATHER_HUMIDITY);
            mHumidity.setText(getString(R.string.format_humidity, humidity));

            double pressure = data.getDouble(INDEX_WEATHER_PRESSURE);
            mPressure.setText(getString(R.string.format_pressure, pressure));

            float windSpeed = data.getFloat(INDEX_WEATHER_WIND_SPEED);
            float windDirection = data.getFloat(INDEX_WEATHER_WIND_DEGREES);
            mWind.setText(SunshineWeatherUtils.getFormattedWind(this, windSpeed, windDirection));
