...
//      COMPLETED (7) Generate a weather summary with the date, description, high and low
        ...
        double highTempC = mCursor.getDouble(MainActivity.INDEX_WEATHER_MAX_TEMP);
        double lowTempC = mCursor.getDouble(MainActivity.INDEX_WEATHER_MIN_TEMP);
        String highAndLowTemperature = SunshineWeatherUtils.formatHighLows(mContext, highTempC, lowTempC);
        String weatherSummaryText = dateString + " - " + description + " - " + highAndLowTemperature;
//      COMPLETED (8) Display the summary that you created above
        forecastAdapterViewHolder.weatherSummary.setText(weatherSummaryText);
    }
